...
- 2026-09-05: At publish time, preserve an explicitly saved cover. If a fresh journey has no saved cover, use its first chronological included photo and persist that choice in the same transaction as the share link and published state. Keep the existing error when no included photo is usable.
- 2026-09-05: Render the landing authentication dialog at the document body so the hero's isolated stacking layer cannot place later landing content above it. Keep the page blocked while the modal is open, use the existing dark green for auth headings, and give mobile Sign in a 44px square minimum hit area.
