...
export function PhotoOnboarding({ existingTripId, existingPhotoCount = 0, reconstructionNeeded = false, onComplete, onCancel, initialError = "" }: {
  ...
  function addFiles(files: File[]) {
    ...
    for (const file of files) {
      ...
      if (persistedByKey.get(key)?.status === "uploaded") { rejected.push(`${file.name} is already safely uploaded.`); continue; }
  ...
  async function queueDraft() {
    ...
    onComplete(existingTripId);
  }

  async function uploadSelected() {
    const candidates = selected.filter((photo) => photo.status !== "saved");
  ...
  return (
    <main className="onboarding-shell core-product">
      ...
      <section className="onboarding-intro" aria-labelledby="onboarding-title">
        <p className="eyebrow">Start with what you already have</p><h1 id="onboarding-title">Choose the photographs from this trip.</h1>
      ...
      <section className="selection-workspace" aria-label="Choose trip photographs">
        <label className="photo-dropzone" onDragOver={(event) => event.preventDefault()} onDrop={drop}>
          <input type="file" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" multiple onChange={(event) => { addFiles(Array.from(event.target.files ?? [])); event.currentTarget.value = ""; }} />
          <span className="dropzone-title">Choose trip photos</span><span>or drag JPEG, PNG, and WebP images here</span>
        </label>
