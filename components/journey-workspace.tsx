...
export function JourneyWorkspace({ trip, onNew, onHome, onSignOut, onManagePhotos, recipientPreview = false, onRecipientPreviewChange }: { trip: Trip; onNew: () => void; onHome: () => void; onSignOut: () => void; onManagePhotos: () => void; recipientPreview?: boolean; onRecipientPreviewChange?: (open: boolean) => void }) {
  ...
  const initialTitle = hasSuggestedTitle && trip.suggestedTitle ? trip.suggestedTitle : trip.title;
  ...
  async function saveIdentity() {
    if (!coverPhotoId || !title.trim() || title.trim().length > MAX_TITLE_LENGTH) return;
    setIdentityStatus(""); setError("");
    await guarded(async () => { try { await confirmTitleAndCover({ tripId: trip._id, title, coverPhotoId }); setIdentityStatus("saved"); setEditingIdentity(false); } catch { setIdentityStatus("error"); } });
  }
  ...
  return (
    <div className="journey-shell core-product">
      ...
      <main className="timeline-workspace">
        ...
        {trip.needsTimelineRebuild ? <section className="rebuild-notice"><div><h2>Update this journey to the new timeline</h2><p>Your uploaded originals stay unchanged.</p></div><button className="primary-button" disabled={busy} onClick={() => void rebuildTimeline()}>{busy ? "Rebuilding…" : "Rebuild timeline"}</button></section> : null}
